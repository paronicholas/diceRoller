...
import {
  ConfigAccessor,
  SFBRequestHandlerFactory,
  UserAgentHelper,
} from "@alexa-games/sfb-skill";
import * as path from "path";

import { ExtensionLoader } from "./extensions/ExtensionLoader";

const projectRootPath = __dirname;
const configAccessor = new ConfigAccessor(
  require(path.resolve("abcConfig", "abcConfig.json")),
  path.resolve(projectRootPath, "res")
);
...
export async function handler(event: any, context: any, callback: any) {
  ...
  const sfbHandler = SFBRequestHandlerFactory.create(
    event,
    context,
    customExtensionLoader.getExtensions(),
    configAccessor,
    projectRootPath
  );
